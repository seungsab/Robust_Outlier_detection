...
class pca_online():
    ...
    def __init__(self, component_use='major', gmm_run=1, n_sample_max=1e4, method_Ncomp='eigengap', method_thresh = 'percentile', cutoff=0.95, n_stall=5, alpha=0.99, n_comp_min=1, n_comp_max=3, cv_types=["spherical", "tied", "diag", "full"], score_type = 'SPE'):
        # BASELINE MODEL
...
#             X.dot(self.pca.components_[:n_comp].T)
    ...
    def compute_threshold(self, score):
        if self.method_thresh == 'conformal_prediction':
